  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            InkWell(
                              onTap: () {
                                final capacity = controller.packageData.value?['capacity'];
                                final maxCapacity = capacity is num ? capacity.toInt() : 999;
                                if (passengerCount.value < maxCapacity) {
                                  passengerCount.value++;
                                }
                              },
                              child: const Padding(
      ...
      body: Obx(() {
        ...
        String statusText = 'สถานะการจอง';
        final now = DateTime.now();

        if (bookingStartDate != null && now.isBefore(bookingStartDate)) {
          statusColor = const Color(0xFFFFB300); // UPCOMING
          statusText = 'เปิดให้จองวันที่ ${controller.formatThaiDate(bookingStartDate)}';
        } else if (bookingEndDate != null && now.isAfter(bookingEndDate)) {
          statusColor = const Color(0xFF94A3B8); // CLOSED
          statusText = 'ปิดจองแล้ว ตั้งแต่ ${controller.formatThaiDate(bookingEndDate)}';
        } else {
          statusColor = const Color(0xFF00C853); // OPEN
          if (bookingStartDate != null && bookingEndDate != null) {
            statusText = 'เปิดจองแล้ว วันที่ ${controller.formatThaiDate(bookingStartDate)} ถึง ${controller.formatThaiDate(bookingEndDate)}';
          } else {
            statusText = 'เปิดจองแล้ว';
          }
        }
                        ...
                        ? PageView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return Image.network(
                                ...
                                fit: BoxFit.cover,
                                errorBuilder: (context, error, stackTrace) => Container(color: Colors.grey[300], child: const Icon(Icons.image, size: 50)),
                              );
                            },
                          )
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // ===== 4. Location Pin =====
                    if (package['address'] != null && package['address'].toString().trim().isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.all(16),
                        child: Row(
                          children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ...homestayPackages.map<Widget>((homestayItem) {
                                ...
                                final homeFacilitiesList = (homestay['facilities'] as List?)
                                        ?.map((f) => f is Map ? f['name']?.toString() ?? '' : '')
                                        .where((name) => name.trim().isNotEmpty)
                                        .toList() ?? [];
                                final homeFacilities = homeFacilitiesList.isNotEmpty ? homeFacilitiesList.join(', ') : '-';
                                        ...
                                        child: Obx(() => Row(
                                          ...
                                          children: List.generate(
                                            ...
                                            (index) => Container(
                                              ...
                                              height: 8,
                                              decoration: BoxDecoration(
                                                color: currentGalleryPage.value == index ? Colors.white : Colors.white.withValues(alpha: 0.5),
                                                shape: BoxShape.circle,
                                              ),
                                            ),
                                          ),
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: facilities.map<Widget>((fac) {
                            final facName = fac is Map ? fac['name']?.toString() ?? '' : '';
                            if (facName.trim().isEmpty) return const SizedBox.shrink();
                            return Padding(
